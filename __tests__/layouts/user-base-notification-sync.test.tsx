...
describe('사용자 알림 레이어 동기화 (gnuboard/g7#14)', () => {
  describe('Initial load hardening', () => {
    it('notifications 목록 데이터소스는 초기 렌더에서 자동 조회하지 않는다', () => {
      const dataSource = userBase.data_sources.find((source) => source.id === 'notifications');

      expect(dataSource).toBeDefined();
      expect(dataSource!.auto_fetch).toBe(false);
